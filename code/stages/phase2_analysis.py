...
def aggregate_demand(root: Path, days: list[str], selected_lines: list[str]) -> pd.DataFrame:
    ...
    for day in days:
        ...
        for chunk in pd.read_csv(
            ...
            registered = chunk.loc[
                chunk["anon_user_id"].map(normalize_code) != "0",
                ["line_code", "service_date", "hour", "anon_user_id"],
            ].dropna(subset=["anon_user_id"]).drop_duplicates()
            registered_user_frames.append(registered)
            grouped = (
    ...
    if registered_user_frames:
        users = (
            pd.concat(registered_user_frames, ignore_index=True)
            .drop_duplicates(["line_code", "service_date", "hour", "anon_user_id"])
            .groupby(["line_code", "service_date", "hour"], as_index=False)
            .agg(distinct_registered_users=("anon_user_id", "size"))
        )
        demand = demand.merge(users, on=["line_code", "service_date", "hour"], how="left")
    else:
        demand["distinct_registered_users"] = 0
    demand["distinct_registered_users"] = demand["distinct_registered_users"].fillna(0).astype(int)
    demand = demand[
        [
            "line_code", "service_date", "hour", "boardings",
            "distinct_registered_users", "free_or_transfer_boardings", "day_type",
        ]
    ]
...
def assemble_hourly(
    ...
    hourly["rain_hour"] = hourly["rain_hour"].astype("boolean").fillna(False).astype(bool)
    hourly["rain_day"] = hourly["rain_day"].astype("boolean").fillna(False).astype(bool)
...
def random_intercept_ml(y: np.ndarray, X: np.ndarray, groups: np.ndarray, names: list[str]) -> dict[str, Any]:
    ...
    def gls_components(log_vars: np.ndarray) -> tuple[float, np.ndarray, np.ndarray, float]:
        ...
        for idx in group_members:
            ...
            x_cross = np.einsum("ni,nj->ij", Xg, Xg, optimize=True)
            xy_cross = np.einsum("ni,n->i", Xg, yg, optimize=True)
            xt_v_x += x_cross / sigma2 - alpha * np.outer(sum_x, sum_x)
            xt_v_y += xy_cross / sigma2 - alpha * sum_x * sum_y
            y_v_y += float(np.sum(yg * yg) / sigma2 - alpha * sum_y * sum_y)
            logdet += (ng - 1) * math.log(sigma2) + math.log(sigma2 + ng * tau2)
        beta = np.linalg.solve(xt_v_x, xt_v_y)
        quad = float(y_v_y - np.sum(beta * xt_v_y))
        ll = -0.5 * (n * math.log(2.0 * math.pi) + logdet + quad)
        return ll, beta, xt_v_x, quad

    y_var = np.var(y) if np.var(y) > 0 else 1.0
    log_variance_bounds = (math.log(1e-12), math.log(1e6))

    def objective(log_vars: np.ndarray) -> float:
        if (
            not np.all(np.isfinite(log_vars))
            or np.any(log_vars < log_variance_bounds[0])
            or np.any(log_vars > log_variance_bounds[1])
        ):
            return 1e100
        try:
...
def ols_fit(y: np.ndarray, X: np.ndarray, k_extra: int = 0) -> dict[str, Any]:
    ...
    # einsum avoids spurious macOS Accelerate matmul overflow warnings observed
    # for this finite, moderately sized fixed-effect design matrix.
    resid = y - np.einsum("ij,j->i", X, beta, optimize=True)
    n = len(y)
...
def save_phase2_figures(*args, **kwargs):
    return None
