...
def scaling_collapse_test(
    ...
    def fit_collapse(
        ...
        def c(params: np.ndarray) -> float:
            ...
            if not (lam.min() <= lc_ <= lam.max()):
                return 1e6
            value = spread((lam - lc_) * nmid ** a_, r_vals * nmid ** b_)
            return value if np.isfinite(value) else 1e6

        exponent_starts = (
            (-2.5, -1.5, -0.5, 0.5, 1.5, 2.5)
            if broad_diagnostic else (-0.5, 0.0, 0.5)
        )
        lambda_quantiles = (
            [0.10, 0.25, 0.50, 0.75, 0.90]
            if broad_diagnostic else [0.25, 0.50, 0.75]
        )
        starts = [
            np.array([a0, b0, lc])
            for a0 in exponent_starts
            for b0 in exponent_starts
            for lc in np.quantile(lam, lambda_quantiles)
        ]
        best_ = min(
    ...
    diag_a, diag_b, diag_lam_c, diag_spread = fit_collapse(
        r, broad_diagnostic=True
    )
    spread_raw = spread(lam, r)
    improvement = spread_raw / spread_scaled if spread_scaled else np.nan
    diag_improvement = spread_raw / diag_spread if diag_spread else np.nan
    optimizer_at_bound = bool(
        max(abs(a), abs(b), abs(diag_a), abs(diag_b)) >= 2.95
    )
    optimizer_stable = bool(
        np.isfinite(spread_scaled)
        and np.isfinite(diag_spread)
        and abs(diag_spread - spread_scaled) <= 0.10 * spread_scaled
    )
    collapse_identified = bool(not optimizer_at_bound and optimizer_stable)
    ...
    summary = {
        ...
        "broad_start_exponent_a": float(diag_a),
        "broad_start_exponent_b": float(diag_b),
        "broad_start_lambda_c": float(diag_lam_c),
        "broad_start_improvement_ratio": float(diag_improvement),
        "optimizer_at_exponent_bound": optimizer_at_bound,
        "optimizer_solution_stable": optimizer_stable,
        "collapse_identified": collapse_identified,
        "verdict": (
            "identified collapse" if collapse_identified else
            "not identified: optimizer boundary/start sensitivity"
        ),
        "note": ("Improvement ratio must be read against the permutation control, "
                 ...
                 "lambda dependence. It must not be interpreted when "
                 "collapse_identified is false."),
    }
...
def main() -> None:
    ...
    parser.add_argument("--outputs-root", type=Path, default=DATA, help="Directory containing phase3/, phase4/, and week1_2/")
    parser.add_argument("--out-dir", type=Path, default=OUT)
    args = parser.parse_args()
    DATA = args.outputs_root.resolve()
