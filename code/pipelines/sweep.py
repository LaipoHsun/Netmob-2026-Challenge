...
def score_grouped_permutation(
    cells: pd.DataFrame,
    bus_hours: pd.DataFrame,
    max_n: int,
    phase_def: str,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Score cells against a line-direction-specific empirical phase null.

    The original report described this sensitivity analysis but the archived
    sweep never called ``grouped_null``.  This implementation preserves each
    line-direction's phase occupancy while breaking within-hour simultaneity.
    """
    phase_col = f"phase_{phase_def}"
    table, samples = nulls.grouped_null(
        bus_hours,
        max_n,
        group_cols=GROUP,
        phase_col=phase_col,
    ...
    scored = cells.merge(
        table[GROUP + ["N", "null_mean", "null_sd"]],
        on=GROUP + ["N"],
        how="inner",
    )
    r = scored[f"r1_{phase_def}"].to_numpy(float)
    n = scored["N"].to_numpy(float)
    scored["r_excess"] = est.r_excess(r, scored["null_mean"].to_numpy(float))
    scored["z"] = est.z_score(
        r,
        scored["null_mean"].to_numpy(float),
        scored["null_sd"].to_numpy(float),
    )
    scored["ppc"] = est.ppc(r, n)
    scored["rayleigh_z"] = est.rayleigh_z(r, n)
    scored["pit_probit"] = np.nan
    for key, idx in scored.groupby(GROUP, sort=False).groups.items():
        key = key if isinstance(key, tuple) else (key,)
        group_samples = samples.get(key)
        if group_samples is None:
            continue
        loc = scored.index.isin(idx)
        scored.loc[loc, "pit_probit"] = est.pit_probit(r[loc], n[loc], group_samples)
    return scored, table


def main() -> None:
    global OUT, DRAWS, BOOT
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--data-dir", type=Path, default=OUT, help="Directory containing cells.csv and bus_hours_with_phases.csv")
    parser.add_argument("--null-draws", type=int, default=DRAWS)
    parser.add_argument("--bootstrap-draws", type=int, default=BOOT)
    args = parser.parse_args()
    OUT = args.data_dir.resolve()
    DRAWS = args.null_draws
    BOOT = args.bootstrap_draws
    ...
    for phase_def in ("arc", "time"):
        ...
        # The fourth null described in the paper is route-conditional and
        # therefore cannot be represented by the pooled NullTable bank above.
        scored, nt = score_grouped_permutation(cells, bus_hours, max_n, phase_def)
        tag = f"{phase_def}|permutation"
        all_contrasts.append(run_contrasts(scored, tag))
        nt["null_kind"] = "permutation"
        nt["phase_def"] = phase_def
        all_nulls.append(nt)
        for e in est.ESTIMATORS:
            sub = scored[["N", e]].dropna()
