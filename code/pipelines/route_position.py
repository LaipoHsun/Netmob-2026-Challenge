...
def terminal_fraction_control(
    scored: pd.DataFrame,
    bus_hours: pd.DataFrame,
) -> dict:
    """Whole-route load model with the cell's terminal occupancy included."""
    terminal_share = (
        bus_hours.assign(
            in_terminal_band=(
                (bus_hours.s_mid_frac <= 0.10) | (bus_hours.s_mid_frac >= 0.90)
            ).astype(float)
        )
        .groupby(CELL, as_index=False)
        .agg(terminal_fraction=("in_terminal_band", "mean"))
    )
    d = scored.merge(terminal_share, on=CELL, how="left")
    d = d[[
        "r_excess", "ppc", "lambda_boardings_per_bus", "N",
        "line_code", "direction", "terminal_fraction",
    ]].dropna()
    d = d.assign(
        lam_z=(d.lambda_boardings_per_bus - d.lambda_boardings_per_bus.mean())
        / d.lambda_boardings_per_bus.std(),
    ...
    for dep in ("r_excess", "ppc"):
        model = smf.ols(
            f"{dep} ~ lam_z + terminal_fraction + logN + C(ld)", data=d
        ).fit(cov_type="cluster", cov_kwds={"groups": d.ld})
        for term in ("lam_z", "terminal_fraction"):
            rows.append({
                "dependent": dep,
                "term": term,
    ...
    table.to_csv(OUT / "terminal_fraction_control.csv", index=False)
    return {
        "n": int(len(d)),
        "r_excess_load_coef": float(
            table.loc[
                (table.dependent == "r_excess") & (table.term == "lam_z"),
                "estimate",
            ].iloc[0]
        ),
        "ppc_load_coef": float(
            table.loc[
                (table.dependent == "ppc") & (table.term == "lam_z"),
                "estimate",
            ].iloc[0]
        ),
    }
...
def main() -> None:
    ...
    parser.add_argument("--outputs-root", type=Path, default=DATA, help="Directory containing week1_2/")
    ...
    terminal_control = terminal_fraction_control(scored, bus_hours)
    ...
    summary = {
        ...
        "terminal_fraction_control": terminal_control,
