...
def load_phase2(root: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, list[str]]:
...
def corridor_coupling_within_pair(
    ...
    phase2_pairs = pd.read_csv(root / "outputs" / "phase2" / "corridor_pair_overlap_and_correlation.csv")
    ...
    if not pair_table.empty:
        ...
        null = signs @ diffs / len(diffs) if len(diffs) else np.array([])
...
def save_figures(
    ...
    mediation_summary: dict[str, Any],
    ...
    for stratum, group in susceptibility.groupby("N_stratum"):
        axes[0].plot(group["lambda_mid"], group["var_r_excess"], marker="o", label=stratum)
    axes[0].set_xlabel("Load per bus lambda")
    axes[0].set_ylabel("Var(r_excess)")
    axes[0].set_title("Susceptibility by N stratum")
    axes[0].grid(True, alpha=0.25)
    axes[0].legend(fontsize=8)
    axes[1].bar(mixture["regime"], mixture["delta_bic_1_minus_2"], color="#4c78a8")
    axes[1].axhline(10, color="black", linestyle="--", linewidth=1)
    axes[1].set_ylabel("BIC(one Gaussian) - BIC(two Gaussian)")
    axes[1].set_title("Bimodality check")
    fig.suptitle(f"P3-1. Criticality gate: {criticality_summary['verdict']}")
    fig.tight_layout()
    fig.savefig(output_dir / "p3_1_criticality_gate.png", dpi=200)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.axis("off")
    a = mediation_summary.get("a_lambda_to_dwell", np.nan)
    b = mediation_summary.get("b_dwell_to_r_excess", np.nan)
    cp = mediation_summary.get("c_prime_direct_lambda_to_r_excess", np.nan)
    ind = mediation_summary.get("indirect_effect", np.nan)
    ci = (mediation_summary.get("indirect_ci_low", np.nan), mediation_summary.get("indirect_ci_high", np.nan))
    ax.text(0.12, 0.55, "lambda\nboardings/N", ha="center", va="center", bbox=dict(boxstyle="round", fc="#d9e8fb", ec="#4c78a8"))
    ax.text(0.50, 0.75, "dwell CV", ha="center", va="center", bbox=dict(boxstyle="round", fc="#fce4d6", ec="#f58518"))
    ax.text(0.88, 0.55, "r_excess", ha="center", va="center", bbox=dict(boxstyle="round", fc="#e1f3df", ec="#54a24b"))
    ax.annotate("", xy=(0.42, 0.72), xytext=(0.20, 0.58), arrowprops=dict(arrowstyle="->", lw=1.8))
    ax.annotate("", xy=(0.80, 0.58), xytext=(0.58, 0.72), arrowprops=dict(arrowstyle="->", lw=1.8))
    ax.annotate("", xy=(0.78, 0.52), xytext=(0.22, 0.52), arrowprops=dict(arrowstyle="->", lw=1.2, linestyle="--"))
    ax.text(0.31, 0.70, f"a={a:.3f}", ha="center")
    ax.text(0.69, 0.70, f"b={b:.3f}", ha="center")
    ax.text(0.50, 0.47, f"direct c'={cp:.3f}", ha="center")
    ax.text(0.50, 0.18, f"indirect a*b={ind:.3f} (95% CI {ci[0]:.3f}, {ci[1]:.3f})", ha="center")
    fig.tight_layout()
    fig.savefig(output_dir / "p3_2_mediation_dwell.png", dpi=200)
    plt.close(fig)

    if not headway_sections.empty:
        hs = headway_sections.merge(
            phase3[["line_code", "direction", "service_date", "hour", "lambda_boardings_per_bus"]],
            on=["line_code", "direction", "service_date", "hour"],
            how="left",
        )
        hs = hs.dropna(subset=["lambda_boardings_per_bus", "headway_cv"]).copy()
        hs["lambda_tertile"] = pd.qcut(hs["lambda_boardings_per_bus"], 3, labels=["low", "mid", "high"], duplicates="drop")
        curve = hs.groupby(["lambda_tertile", "section_frac"], observed=True, as_index=False)["headway_cv"].mean()
    else:
        curve = pd.DataFrame()
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    for tertile, group in curve.groupby("lambda_tertile", observed=True):
        axes[0].plot(group["section_frac"], group["headway_cv"], marker="o", label=str(tertile))
    axes[0].set_xlabel("Arc-length fraction")
    axes[0].set_ylabel("Mean headway CV")
    axes[0].set_title("Headway-CV by demand tertile")
    axes[0].legend()
    axes[0].grid(True, alpha=0.25)
    slopes = phase3["headway_cv_slope"].dropna()
    axes[1].hist(slopes, bins=40, color="#4c78a8", alpha=0.8)
    axes[1].axvline(0, color="black", linewidth=1)
    axes[1].set_title(f"Slope distribution, mean={spatial_summary.get('mean_slope', np.nan):.3f}")
    axes[1].set_xlabel("Headway-CV slope vs arc length")
    fig.tight_layout()
    fig.savefig(output_dir / "p3_3_spatial_amplification.png", dpi=200)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(7, 5))
    if not corridor_pairs.empty:
        x = np.arange(len(corridor_pairs))
        ax.scatter(np.zeros(len(corridor_pairs)), corridor_pairs["shared_corr"], alpha=0.6, label="shared")
        ax.scatter(np.ones(len(corridor_pairs)), corridor_pairs["offtrunk_corr"], alpha=0.6, label="off-trunk")
        for _, row in corridor_pairs.iterrows():
            ax.plot([0, 1], [row["shared_corr"], row["offtrunk_corr"]], color="gray", alpha=0.25)
    ax.set_xticks([0, 1])
    ax.set_xticklabels(["shared trunk", "off-trunk"])
    ax.set_ylabel("Cross-line corr(r_excess)")
    ax.set_title("P3-4. Within-pair corridor coupling")
    ax.grid(True, axis="y", alpha=0.25)
    fig.tight_layout()
    fig.savefig(output_dir / "p3_4_corridor_coupling_within_pair.png", dpi=200)
    plt.close(fig)

    eligible = phase3[phase3["analysis_eligible"]].dropna(subset=["lambda_boardings_per_bus", "r_excess"]).copy()
    eligible["lambda_bin"] = pd.qcut(eligible["lambda_boardings_per_bus"], 30, duplicates="drop")
    binned = (
        eligible.groupby("lambda_bin", observed=True, as_index=False)
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.scatter(binned["lambda_mid"], binned["mean_r_excess"], s=np.clip(binned["rows"], 20, 180), alpha=0.75)
    ax.plot(xgrid, beta[0] + beta[1] * xgrid, color="#d62728", linewidth=2, label="linear/crossover summary")
    ax.axhline(0, color="black", linewidth=0.8)
    ax.set_xlabel("Load per bus lambda")
    ax.set_ylabel("Corrected synchronization r_excess")
    ax.set_title("P3-5. Demand coupling after criticality gate")
    ax.legend()
    ax.grid(True, alpha=0.25)
    fig.tight_layout()
    fig.savefig(output_dir / "p3_5_updated_demand_coupling.png", dpi=200)
    plt.close(fig)
...
"""
    (root / "phase3_report.md").write_text(report)
...
def main() -> None:
    ...
    hourly, bus_hours, line_selection, null_table, selected_lines = load_phase2(root)
    ...
    segment_table, corridor_pairs, segment_summary, corridor_summary = corridor_coupling_within_pair(
        root, output_dir, hourly, bus_hours, selected_lines, static_geoms, gtfs_geoms, null_table
    )
