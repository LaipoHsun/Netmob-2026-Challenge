...
def combine_parts(parts: list[list[tuple[float, float]]]) -> list[tuple[float, float]]:
    """Flatten route parts without adding synthetic connector points."""
...
def load_static_geometries(root: Path) -> dict[tuple[str, int | None], RouteGeometry]:
...
def save_figures(
    ...
    figure_info: dict[str, object] = {}
    busy, quiet = choose_busy_quiet(demand)
    figure_info["busy_line_for_figures"] = busy
    figure_info["quiet_line_for_figures"] = quiet

    f1 = resampled[
        (resampled["service_date"] == FOCUS_DAYS[0]) & (resampled["line_code"].isin([busy, quiet]))
    ].copy()
    f1 = f1[["line_code", "direction", "vehicle_id", "timestamp", "s_frac", "s_m", "route_length_m"]]
    f1.to_csv(output_dir / "f1_marey_data.csv", index=False)
    fig, axes = plt.subplots(1, 2, figsize=(14, 5), sharey=True)
    for ax, line in zip(axes, [busy, quiet]):
        data = f1[f1["line_code"] == line].sort_values("timestamp")
        for (_, direction, vehicle), group in data.groupby(["line_code", "direction", "vehicle_id"], sort=False):
            ax.plot(group["timestamp"], group["s_frac"], linewidth=0.45, alpha=0.28)
        ax.set_title(f"Line {line} on {FOCUS_DAYS[0]}")
        ax.set_xlabel("Local time (BRT)")
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
        ax.grid(True, alpha=0.25)
    axes[0].set_ylabel("Arc-length fraction")
    fig.suptitle("F1. Time-space diagrams")
    fig.tight_layout()
    fig.savefig(output_dir / "f1_marey_busy_vs_quiet.png", dpi=200)
    plt.close(fig)

    f2_events = events.copy()
    f2_events.to_csv(output_dir / "f2_headway_events.csv", index=False)
    cv_ts = (
        tidy.groupby(["line_code", "service_date", "time_bin"], as_index=False)
        .agg(headway_cv=("headway_cv", "mean"), bunching_rate=("bunching_rate", "mean"))
        .dropna(subset=["headway_cv"])
    )
    cv_ts.to_csv(output_dir / "f2_cv_timeseries.csv", index=False)
    fig, axes = plt.subplots(1, 2, figsize=(15, 5))
    data = events[events["headway_min"].between(0, 60)].copy()
    if not data.empty:
        axes[0].boxplot(
            [data.loc[data["line_code"] == line, "headway_min"].dropna().to_numpy() for line in FOCUS_LINES],
            tick_labels=FOCUS_LINES,
            showfliers=False,
        )
    axes[0].set_title("Observed headways by focus line")
    axes[0].set_ylabel("Headway (minutes)")
    axes[0].grid(True, axis="y", alpha=0.25)
    for line, group in cv_ts.groupby("line_code"):
        axes[1].plot(group["time_bin"], group["headway_cv"], label=line, linewidth=1.2)
    axes[1].set_title("Rolling 30 min headway CV")
    axes[1].set_ylabel("CV")
    axes[1].set_xlabel("Local time (BRT)")
    axes[1].xaxis.set_major_formatter(mdates.DateFormatter("%m-%d %H:%M"))
    axes[1].grid(True, alpha=0.25)
    axes[1].legend(ncol=3, fontsize=8)
    fig.suptitle("F2. Headway distribution and CV")
    fig.tight_layout()
    fig.savefig(output_dir / "f2_headway_distribution_cv.png", dpi=200)
    plt.close(fig)

    line_ts = (
        tidy.groupby(["line_code", "service_date", "time_bin"], as_index=False)
        .agg(kuramoto_r=("kuramoto_r", "mean"), active_buses=("active_buses", "sum"), boardings=("boardings", "mean"))
    )
    f3 = line_ts[(line_ts["service_date"] == FOCUS_DAYS[0]) & (line_ts["line_code"].isin([busy, quiet]))].copy()
    f3.to_csv(output_dir / "f3_order_demand_data.csv", index=False)
    fig, axes = plt.subplots(2, 1, figsize=(13, 7), sharex=True)
    for ax, line in zip(axes, [busy, quiet]):
        group = f3[f3["line_code"] == line].sort_values("time_bin")
        ax.plot(group["time_bin"], group["kuramoto_r"], color="#1f77b4", label="Kuramoto r", linewidth=1.8)
        ax.set_ylim(0, 1.05)
        ax.set_ylabel("r")
        ax.grid(True, alpha=0.25)
        twin = ax.twinx()
        twin.plot(group["time_bin"], group["boardings"], color="#d62728", alpha=0.65, label="Boardings")
        twin.set_ylabel("Boardings / 10 min")
        for start_hour, end_hour in [(7, 9), (17, 19)]:
            start = pd.Timestamp(f"{FOCUS_DAYS[0]} {start_hour:02d}:00")
            end = pd.Timestamp(f"{FOCUS_DAYS[0]} {end_hour:02d}:00")
            ax.axvspan(start, end, color="#f0c36d", alpha=0.18)
        ax.set_title(f"Line {line}")
    axes[-1].xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
    axes[-1].set_xlabel("Local time (BRT)")
    fig.suptitle("F3. Order parameter and boarding demand")
    fig.tight_layout()
    fig.savefig(output_dir / "f3_order_parameter_demand.png", dpi=200)
    plt.close(fig)

    scatter = tidy.copy()
    scatter["hour_bin"] = scatter["time_bin"].dt.floor("1h")
    f4 = (
        scatter.groupby(["line_code", "service_date", "day_category", "hour_bin"], as_index=False)
        .agg(
            mean_r=("kuramoto_r", "mean"),
            mean_boardings_per_10min=("boardings", "mean"),
            mean_headway_cv=("headway_cv", "mean"),
            mean_active_buses=("active_buses", "mean"),
        )
        .dropna(subset=["mean_r"])
    )
    f4.to_csv(output_dir / "f4_cross_line_scatter_data.csv", index=False)
    fig, ax = plt.subplots(figsize=(8, 6))
    colors = {"weekday": "#1f77b4", "weekend": "#ff7f0e"}
    for cat, group in f4.groupby("day_category"):
        ax.scatter(
            group["mean_boardings_per_10min"],
            group["mean_r"],
            s=24 + 3 * group["mean_active_buses"].fillna(0),
            alpha=0.7,
    ...
    ax.set_xlabel("Mean boardings per 10 min")
    ax.set_ylabel("Mean Kuramoto r")
    ax.set_ylim(0, 1.05)
    ax.grid(True, alpha=0.25)
    ax.legend(title="Day type")
    fig.suptitle("F4. Demand vs synchronization first look")
    fig.tight_layout()
    fig.savefig(output_dir / "f4_demand_vs_order_scatter.png", dpi=200)
    plt.close(fig)
...
"""
    (root / "phase1_report.md").write_text(report)
